/* -[ClockSketch v7.2]----------------------------------------------------------------------------------------
   ...
   NOTE: 6 DIGIT VERSION NEEDS DEBUG DISABLED TO SAVE RAM ON ARDUINOS!
...
// comment below to disable serial in-/output and free some RAM -> REQUIRED to disable this on Lazy 7 / One with 6 digits on Arduino, RAM usage!
#define DEBUG
...
// #define SKETCHNAME "v7.2"                           // shortening some stuff to save precious bytes of ram ^^
// #define CLOCKNAME "L7/O"                          // ...this actually gave 20 bytes, that's worth ~6 leds! ;-D
...
#define LEDSTUFF
  ...
  #define LED_COUNT 252                                          // Total number of leds, 252 on Lazy 7 / One, 4 digits
  #if ( LED_DIGITS == 6 )
    #define LED_COUNT 388                                        // 388 leds on the 6 digit version of Lazy 7 / One
  #endif
...
/* Below is the configuration for led <> segment assignments.
   ...
   LED_ACCESS_MODE 2 will use the first two and the last two values inside each
   segment (led a, led b) as start and end value to get 2+ leds/segment.

   Example:
   leds 0, 3 -> MODE 0 -> led 0 and 3 inside the segment -> 2 leds
   leds 0, 3 -> MODE 1 -> led 0 - 3 inside the segment -> 4 leds
   leds 0, 3, 20, 23 -> MODE 2 -> leds 0-3 and 20-23 inside the segment -> 8 leds
...
// uint8_t digitsLAM[6] = { 2, 2, 2, 2, 2, 2 };

// !!!!!!!!!! disabled LAM mode for l7/one completely, need to save some ram for the 6 digit version... !!!!!!!!!!!!!!

#if ( LED_DIGITS == 4 )
  const uint8_t digitPositions[4] = { 3, 2, 1, 0 };                 // positions of HH:MM (3, 0, 2, 1 on L7-QBE)
  const uint16_t segGroups[28][4] PROGMEM = {
    ...
    /* digit position 1 */
    {   34,  37,  46,  49 },                     // top, a
    {   30,  33,  50,  53 },                     // top right, b
    {  202, 205, 206, 209 },                     // bottom right, c
    {  198, 201, 210, 213 },                     // bottom, d
    {  194, 197, 214, 217 },                     // bottom left, e
    {   38,  41,  42,  45 },                     // top left, f
    {   54,  57, 218, 221 },                     // center, g
    /* digit position 2 */
    {   72,  75,  84,  87 },                     // top, a
    {   68,  71,  88,  91 },                     // top right, b
    {  164, 167, 168, 171 },                     // bottom right, c
    {  160, 163, 172, 175 },                     // bottom, d
    {  156, 159, 176, 179 },                     // bottom left, e
    {   76,  79,  80,  83 },                     // top left, f
    {   92,  95, 180, 183 },                     // center, g
    /* digit position 3 */
    {  102, 105, 114, 117 },                     // top, a
    {   98, 101, 118, 121 },                     // top right, b
    {  134, 137, 138, 141 },                     // bottom right, c
    {  130, 133, 142, 145 },                     // bottom, d
    {  126, 129, 146, 149 },                     // bottom left, e
    {  106, 109, 110, 113 },                     // top left, f
    {  122, 125, 150, 153 },                     // center, g
  };
#endif

#if ( LED_DIGITS == 6 )
  const uint8_t digitPositions[6] = { 5, 4, 3, 2, 1, 0 };           // positions of HH:MM:SS
  const uint16_t segGroups[42][4] PROGMEM = {
    /* segments 0-42, 6 digits x 7 segments */
    /* digit position 0 */
    {   4,   7,  16,  19 },                     // top, a
    {   0,   3,  20,  23 },                     // top right, b
    ...
    {  34,  37,  46,  49 },                     // top, a
    {  30,  33,  50,  53 },                     // top right, b
    { 338, 341, 342, 345 },                     // bottom right, c
    { 334, 337, 346, 349 },                     // bottom, d
    { 330, 333, 350, 353 },                     // bottom left, e
    {  38,  41,  42,  45 },                     // top left, f
    {  54,  57, 354, 357 },                     // center, g
    /* digit position 2 */
    {  72,  75,  84,  87 },                     // top, a
    {  68,  71,  88,  91 },                     // top right, b
    { 300, 303, 304, 307 },                     // bottom right, c
    { 296, 299, 308, 311 },                     // bottom, d
    { 292, 295, 312, 315 },                     // bottom left, e
    {  76,  79,  80,  83 },                     // top left, f
    {  92,  95, 316, 319 },                     // center, g
    /* digit position 3 */
    { 102, 105, 114, 117 },                     // top, a
    {  98, 101, 118, 121 },                     // top right, b
    { 270, 273, 274, 277 },                     // bottom right, c
    { 266, 269, 278, 281 },                     // bottom, d
    { 262, 265, 282, 285 },                     // bottom left, e
    { 106, 109, 110, 113 },                     // top left, f
    { 122, 125, 286, 289 },                     // center, g
    /* digit position 4 */
    { 140, 143, 152, 155 },                   // top, a
    { 136, 139, 156, 159 },                   // top right, b
    { 232, 235, 236, 239 },                   // bottom right, c
    { 228, 231, 240, 243 },                   // bottom, d
    { 224, 227, 244, 247 },                   // bottom left, e
    { 144, 147, 148, 151 },                   // top left, f
    { 160, 163, 248, 251 },                   // center, g
    /* digit position 5 */
    { 170, 173, 182, 185 },                   // top, a
    { 166, 169, 186, 189 },                   // top right, b
    { 202, 205, 206, 209 },                   // bottom right, c
    { 198, 201, 210, 213 },                   // bottom, d
    { 194, 197, 214, 217 },                   // bottom left, e
    { 174, 177, 178, 181 },                   // top left, f
    { 190, 193, 218, 221 }                    // center, g
  };
#endif

#if ( LED_DIGITS == 4 )
  const uint16_t upperDots[6] PROGMEM = {  60,  61,  62,  63,  64,  65 };              // leds inside the upper dots (1st and last on L7/O)
  const uint16_t lowerDots[6] PROGMEM = { 186, 187, 188, 189, 190, 191 };              // leds inside the lower dots (1st and last on L7/O)
#endif

#if ( LED_DIGITS == 6 )
  const uint16_t upperDots[12] PROGMEM = {  60,  61,  62,  63,  64,  65, 128, 129, 130, 131, 132, 133 };     // all the leds inside the upper dots (bogus values on some models which don't support 6 digits)13
  const uint16_t lowerDots[12] PROGMEM = { 322, 323, 324, 325, 326, 327, 254, 255, 256, 257, 258, 259 };     // all the leds inside the lower dots (bogus values on some models which don't support 6 digits)
#endif
...
void setup() {
    ...
    #endif
      Serial.println(F("Lazy 7 / One - ClockSketch v7.2 starting..."));
    #ifdef LEDSTUFF
...
void fadeSegment(uint8_t pos, uint8_t segment, uint8_t amount, uint8_t fadeType) {
  /* this  will fade by "amount" using "fadeType". "fadeType" is important because when
     fading things in that were off previously we must avoid setting them black at
     first - hence "fadeLightBy" instead of "fadeToBlack".                                */
  if ( fadeDigits == 0 ) return;
  segment += pos * 7;
  uint16_t startLedA = pgm_read_word_near(&segGroups[segment][0]);
  uint16_t endLedA = pgm_read_word_near(&segGroups[segment][1]);
  uint16_t startLedB = pgm_read_word_near(&segGroups[segment][2]);
  uint16_t endLedB = pgm_read_word_near(&segGroups[segment][3]);
  for ( uint16_t i = startLedA; i <= endLedA; i++ ) {
    if ( fadeType == 0 ) {
  ...
  for ( uint16_t i = startLedB; i <= endLedB; i++ ) {
    if ( fadeType == 0 ) {
      leds[i].fadeToBlackBy(amount);
    } else {
      leds[i].fadeLightBy(amount);
...
void dotsFader() {
  ...
  if ( fadeDots == 2 && active ) {                            // fade in/out dots
    ...
    if ( lastRun - firstRun >= 900 - fadeDelay * fadeSteps ) {
      for ( uint8_t i = 0; i < ( sizeof(upperDots) / sizeof(upperDots[0]) ); i++ ) {
...
/* Checks if segment at position is on - and if it is, assigns color from current palette  */
  if ( leds[pgm_read_word_near(&segGroups[segment + digitPositions[pos] * 7][0])] ) {
    uint16_t startLedA = pgm_read_word_near(&segGroups[segment + digitPositions[pos] * 7][0]);
    uint16_t endLedA = pgm_read_word_near(&segGroups[segment + digitPositions[pos] * 7][1]);
    uint16_t startLedB = pgm_read_word_near(&segGroups[segment + digitPositions[pos] * 7][2]);
    uint16_t endLedB = pgm_read_word_near(&segGroups[segment + digitPositions[pos] * 7][3]);
    for ( uint16_t i = startLedA; i <= endLedA; i++ ) {
...
/* Used for coloring digits inside setup routines/steps
  ...
  if ( leds[pgm_read_word_near(&segGroups[segment + pos * 7][0])] ) {                                // if first led inside segment is lit...
      uint16_t startLedA = pgm_read_word_near(&segGroups[segment + pos * 7][0]);
      uint16_t endLedA = pgm_read_word_near(&segGroups[segment + pos * 7][1]);
      uint16_t startLedB = pgm_read_word_near(&segGroups[segment + pos * 7][2]);
      uint16_t endLedB = pgm_read_word_near(&segGroups[segment + pos * 7][3]);
      for ( uint16_t i = startLedA; i <= endLedA; i++ ) {                                              // assign hue to led 0 - 1 inside segment
        leds[i].setHSV(hue, sat, bri);
      }
      for ( uint16_t i = startLedB; i <= endLedB; i++ ) {                                              // assign hue to led 0 - 1 inside segment
        leds[i].setHSV(hue, sat, bri);
      }
    }
  }
...
void showSegment(uint8_t segment, uint8_t segDisplay) {
  ...
  // "firstLoop" is used to display all information only once per test if customHelper is defined
      ...
      if ( firstLoop ) {
        ...
        Serial.print(F(" is 2")); // hard coded on l7/one, need ram for 6 digit version....
      }
    #endif
  #endif
  ...
  uint16_t endLedB = pgm_read_word_near(&segGroups[segment][3]);
    ...
    #ifdef CUSTOMHELPER
      if ( firstLoop ) {
        Serial.print(F(". Leds ")); Serial.print(startLedA);
        Serial.print(F(" - ")); Serial.print(endLedA);
        Serial.print(F(" and leds ")); Serial.print(startLedB);
        Serial.print(F(" - ")); Serial.println(endLedB);
      }
    #endif
  #endif
  for ( uint16_t i = startLedA; i <= endLedA; i++ ) {
    leds[i].setHSV(0, 255, 30);
  }
  for ( uint16_t i = startLedB; i <= endLedB; i++ ) {
    leds[i].setHSV(0, 255, 30);
  }
...
void showDots(uint8_t dots) {
  ...
  if ( dots == 1  || dots == 2 ) {
    for ( uint8_t i = 0; i < ( sizeof(upperDots) / sizeof(upperDots[0]) ); i++ ) {
      leds[pgm_read_word_near(&upperDots[i])].setHSV(0, 255, 30);
    }
  }
  if ( dots == 0 || dots == 2 ) {
    for ( uint8_t i = 0; i < ( sizeof(lowerDots) / sizeof(lowerDots[0]) ); i++ ) {
      leds[pgm_read_word_near(&lowerDots[i])].setHSV(0, 255, 30);
    }
  }
}
...
/* This assists in troubleshooting and basic configuration. Testing all neccessary steps to get
  ...
  void customHelper() {
    ...
    while ( test == 1 ) {
      for ( uint16_t i = 0; i < LED_COUNT; i++ ) {
        leds[i].setHSV(96, 255, brightness);
        FastLED.show();
        delay(40);
